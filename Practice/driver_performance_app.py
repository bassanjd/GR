...
def load_track(path: str) -> pd.DataFrame:
    ...
    Dispatches to the correct loader based on filename:
      RaceBox*        → load_racebox   (raw ISO-8601, haversine distance)
      *speed_tracker* → load_speed_tracker  (already standard schema)
      everything else → load_generic   (Generic datalogger: GPS+IMU+temps)
    """
    return loaders.load_any(path)
...
track_files = sorted(DATALOGS.glob("*.parquet"), key=lambda p: p.name, reverse=True)
if not track_files:
    st.error(f"No parquet files found in {DATALOGS}")
    st.stop()

# ── Settings defaults via session state (Settings tab widgets write here) ─────
for _k, _v in [("cfg_lat", 29.33715), ("cfg_smooth", 9),
